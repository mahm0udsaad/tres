  ...
  return <>
    ...
    {showCreateForm ? <section className="staff-card staff-checklist-form-card">
      ...
      {employees.length === 0 ? <div className="staff-empty staff-empty-action">أنشئ حسابات الموظفين أولاً.<Link href="/staff/owner/team">فتح الموظفين</Link></div> : <form className="staff-form staff-checklist-form" action={(form) => startTransition(() => createAction(form))}>
        ...
        <label className="staff-checklist-check"><input name="task_repeat_daily" type="checkbox" defaultChecked /><span><Repeat /> تتكرر تلقائياً كل يوم</span></label>
    ...
    <section className="staff-card staff-checklist-list">
      <div className="staff-card-head"><div><h2>المهام الحالية</h2><p>أعد إسناد المهمة أو عدّلها أو احذفها قبل أن يكملها الموظف.</p></div><span className="staff-team-count">{tasks.length}</span></div>
      ...
      {tasks.length ? <ul className="staff-checklist-items">{tasks.map((task) => { const employee = employees.find((item) => item.user_id === task.user_id); return <li key={task.id}><div className="staff-checklist-item-info"><strong>{task.title}{task.recurring_assignment_id ? <small className="staff-task-repeat-badge"><Repeat /> يومية</small> : null}</strong><span>{employee?.full_name ?? "موظف"} · {task.task_date}{task.task_date < today ? " · متأخرة" : ""}</span>{task.notes ? <small>{task.notes}</small> : null}</div><div className="staff-checklist-item-actions"><button type="button" className="staff-checklist-edit staff-checklist-assign" onClick={() => setAssigningTask(task)} aria-label={`إسناد ${task.title} لموظف`} title="نسخ لموظف آخر"><Repeat /></button><button type="button" className="staff-checklist-edit" onClick={() => setEditing(task)} aria-label="تعديل المهمة" title="تعديل"><Pencil /></button><form action={(form) => startTransition(() => deleteAction(form))}><input type="hidden" name="task_id" value={task.id} /><button className="staff-checklist-edit staff-checklist-off" disabled={deleting} aria-label={task.recurring_assignment_id ? "إيقاف المهمة اليومية" : "حذف المهمة"} title={task.recurring_assignment_id ? "إيقاف اليومية" : "حذف"}><Trash2 /></button></form></div></li>; })}</ul> : <p className="staff-empty">لا توجد مهام. ابدأ بإنشاء أول مهمة أعلاه.</p>}
    </section>
    ...
    {editing ? <div className="owner-member-modal-backdrop" role="presentation" onMouseDown={() => setEditing(null)}><section className="owner-member-modal" role="dialog" aria-modal="true" onMouseDown={(event) => event.stopPropagation()}><header><div><p className="staff-eyebrow">EDIT TASK</p><h2>تعديل المهمة</h2></div><button type="button" className="staff-icon-button" onClick={() => setEditing(null)}><X /></button></header><form className="staff-form staff-checklist-form" action={(form) => startTransition(() => updateAction(form))} onSubmit={() => setEditing(null)}>
      ...
      <label className="staff-checklist-check"><input name="task_repeat_daily" type="checkbox" defaultChecked={Boolean(editing.recurring_assignment_id)} /><span><Repeat /> تتكرر تلقائياً كل يوم</span></label>
  ...
  return <div className="owner-member-modal-backdrop" role="presentation" onMouseDown={onClose}>
    <section className="owner-member-modal owner-task-assign-modal" role="dialog" aria-modal="true" aria-labelledby="repeat-task-title" onMouseDown={(event) => event.stopPropagation()}>
      ...
      <form className="staff-form staff-checklist-form" action={(form) => startTransition(() => action(form))}>
        ...
        <button type="submit" className="staff-primary staff-field-wide" disabled={pending}><Repeat /> {pending ? "جارٍ الإسناد…" : "تأكيد الإسناد"}</button>
      </form>
    </section>
  </div>;
}
