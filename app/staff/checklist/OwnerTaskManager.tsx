...
export default function OwnerTaskManager({ employees, tasks, branchNames }: {
  employees: Employee[];
  tasks: Task[];
  ...
  return <>
    <section className="staff-card staff-checklist-form-card">
      <div className="staff-card-head"><div><p className="staff-eyebrow">OWNER ASSIGNMENT</p><h2>إنشاء وتوزيع مهمة</h2><p>اكتب أي مهمة من الصفر واختر الموظفين. لن تظهر أي مهمة لم يوزعها المالك.</p></div><Users className="staff-checklist-head-icon" /></div>
      {employees.length === 0 ? <p className="staff-empty">أنشئ حسابات الموظفين أولاً.</p> : <form className="staff-form staff-checklist-form" action={(form) => startTransition(() => createAction(form))}>
    ...
    <section className="staff-card staff-checklist-list">
      <div className="staff-card-head"><div><p className="staff-eyebrow">PENDING</p><h2>المهام الحالية</h2><p>يمكن تعديل المهمة أو حذفها من الموظف قبل إكمالها.</p></div><span className="staff-team-count">{tasks.length}</span></div>
